...
class TimeFieldTest extends BrowserTestBase {
  ...
  public function testTimeFieldRequired($time, $expected = NULL, $error = NULL) {
    ...
    // Otherwise, check for the expected value.
      ...
      $this->assertSession()->elementTextEquals('css', 'article > div > div', $expected);
    }
  }
  ...
  public function testTimeField($time, $expected = NULL, $error = NULL) {
    ...
    // Otherwise, check for the expected value.
      ...
      $this->assertSession()->addressMatches('/^\/node\/\d$/');
      if (empty($expected)) {
        $this->assertSession()->elementNotExists('css', 'article > div > div');
      }
      else {
        $this->assertSession()->elementTextEquals('css', 'article > div > div', $expected);
      }
   ...
   */
  public static function timeFieldRequiredDataProvider() {
    return [
      // Correct time values.
      ['00:00', '12:00 am'],
      ['03:30', '03:30 am'],
      ...
      [NULL, NULL, 'field_test_time field is required.'],
      ['', '', 'field_test_time field is required.'],
    ];
  }
   ...
   *   An array of test data.
   */
  public static function timeFieldDataProvider() {
    return [
      // Correct time values.
      ['00:00', '12:00 am'],
      ['03:30', '03:30 am'],
  ...
  /**
   * Tests the default value options for the time field.
   */
  public function testTimeFieldDefaultValue() {
    // Set the field to use current time as default value.
    $this->drupalGet('admin/structure/types/manage/' . $this->contentType . '/fields/node.' . $this->contentType . '.' . $this->fieldName);
    $this->submitForm([
      'set_default_value' => '1',
      'default_value_input[' . $this->fieldName . '][0][value]' => '',
      'default_value_input[use_current_time]' => '1',
    ], 'Save settings');
    $this->assertSession()->statusCodeEquals(200);

    // Go to the node creation page and make sure the default value is correct.
    $this->drupalGet('node/add/' . $this->contentType);
    $this->assertSession()->statusCodeEquals(200);
    $this->assertSession()->fieldValueEquals($this->fieldName . '[0][value]', date('H:i'));

    // Set the field to use predefined time as a value.
    $this->drupalGet('admin/structure/types/manage/' . $this->contentType . '/fields/node.' . $this->contentType . '.' . $this->fieldName);
    $this->submitForm([
      'set_default_value' => '1',
      'default_value_input[' . $this->fieldName . '][0][value]' => '10:40',
      'default_value_input[use_current_time]' => '0',
    ], 'Save settings');
    $this->assertSession()->statusCodeEquals(200);
    ...
    $this->drupalGet('node/add/' . $this->contentType);
    $this->assertSession()->statusCodeEquals(200);
    $this->assertSession()->fieldValueEquals($this->fieldName . '[0][value]', '10:40');
  }
